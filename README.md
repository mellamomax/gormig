...
- Apify TikTok discovery source behind a `SocialMediaSource` interface
- Apify transcript actor for new TikTok videos
...
- analyzing transcripts generated by Apify

Use a low monthly API budget/limit if you want cost control.

### Apify

Apify is only needed for manual TikTok scraping/backfill and automatic transcript generation for new videos. Manual transcript testing does not need Apify.

Use one actor for discovery (`APIFY_ACTOR_ID`) and one actor for transcription (`APIFY_TRANSCRIPT_ACTOR_ID`). Different TikTok Apify actors use slightly different input/output fields. This app supports common discovery fields and common transcript output fields. You can override inputs with `APIFY_ACTOR_INPUT_JSON` and `APIFY_TRANSCRIPT_ACTOR_INPUT_JSON`.
...
APIFY_TRANSCRIPT_ACTOR_INPUT_JSON=
```
